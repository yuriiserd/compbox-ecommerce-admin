...
export default function Home() {
   ...
   const firstDayOfMonth = useMemo(():Date => {
      const date = new Date();
   ...
   const ordersCount = {};
   const ordersTotal = {};
   const sordedOrders = orders.sort((order1: Order, order2: Order) => (
      new Date(order1.createdAt).getTime() - new Date(order2.createdAt).getTime()
   ));
   sordedOrders.forEach((order: Order) => {
      const date = new Date(order.createdAt);
   ...
   const sordedCustomers = customers.sort((customer1: Customer, customer2: Customer) => (
      new Date(customer1.createdAt).getTime() - new Date(customer2.createdAt).getTime()
   ));
   sordedCustomers.forEach((customer: Customer) => {
